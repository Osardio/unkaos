...
export default {
  ...
  computed: {
    getAvatar() {
      return this.$store.state.users?.users?.find(user => user.uuid === this.userUuid)?.avatar ?? this.defaultAvatar;
    }
  }
}
</script>
